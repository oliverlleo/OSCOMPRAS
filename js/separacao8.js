// INÍCIO DO ARQUIVO js/separacao8.js
// ESTE É O ARQUIVO SEPARACAO8.JS (para verificação e teste final)

let tabelaCorrecao = null; // Variável global para a DataTable

document.addEventListener("DOMContentLoaded", () => {
    console.log("DOM completamente carregado (separacao8.js).");
    ...
    document.getElementById("selectLista").addEventListener("change", async () => {
        ...
        if (clienteId && tipoProjeto && nomeListaOriginal) {
            // Apenas exibe os itens elegíveis, não salva em CorrecaoFinal aqui.
            await exibirItensElegiveisDaListaOriginal(clienteId, tipoProjeto, nomeListaOriginal);
        }
    });

    document.getElementById("btnGerar").addEventListener("click", gerarSeparacaoFinal); // Renomeado para clareza
    ...
    if (btnGerarNecessidade) {
        btnGerarNecessidade.addEventListener("click", async () => {
            ...
            try {
                ...
                console.error("Erro ao carregar itens para necessidade de compra (separacao8.js):", error);
    ...
    document.getElementById("btnConfirmarNecessidade").addEventListener("click", async () => {
        ...
        try {
            ...
            console.error("Erro ao confirmar necessidade de compra (separacao8.js):", error);
    ...
    if (typeof $ !== "undefined" && $.fn && $.fn.DataTable) {
        ...
        console.error("jQuery ou DataTables não estão carregados (separacao8.js).");
    }
});
...
function carregarClientes() {
    ...
    firebase.database().ref("clientes").once("value")
        ...
        }).catch((err) => console.error("Erro ao carregar clientes (separacao8.js):", err));
}
...
function carregarTiposProjeto() {
    ...
    firebase.database().ref(`projetos/${clienteId}`).once("value")
        .then((snap) => {
            ...
            if (typeof $ !== "undefined" && $.fn && $.fn.select2 && $(sel).data("select2")) $(sel).trigger("change");
        }).catch((err) => console.error("Erro ao carregar tipos de projeto (separacao8.js):", err));
}
...
async function carregarListas() {
    ...
    try {
        ...
        console.error("Erro ao carregar listas (separacao8.js):", err);
        if (typeof mostrarNotificacao === "function") mostrarNotificacao("Falha ao carregar listas.", "danger");
    }
}

// Renomeada e refatorada para carregar itens da lista original na tabela e salvar em CorrecaoFinal
async function processarListaSelecionadaParaTabela(clienteId, tipoProjeto, nomeListaOriginal) {
    if (!clienteId || !tipoProjeto || !nomeListaOriginal) {
        if (tabelaCorrecao) tabelaCorrecao.clear().draw();
        return;
    }
    console.log(`[processarListaSelecionadaParaTabela] Para ${clienteId}/${tipoProjeto}/${nomeListaOriginal} (separacao8.js)`);
    try {
        ...
        if (!snapshotLista.exists()) {
            ...
            await refCorrecaoFinalFallback.set([]); // Limpa CorrecaoFinal se lista original não existe
            return;
        }
        ...
        console.error("Erro em processarListaSelecionadaParaTabela (separacao8.js):", error);
...
async function compararEProcessarListas(clienteId, tipoProjeto, nomeListaOriginal) {
    console.log(`[compararEProcessarListas] Iniciando para ${clienteId}/${tipoProjeto}/${nomeListaOriginal} (separacao8.js)`);
    ...
    if (itensBase.length === 0) {
        console.warn("[compararEProcessarListas] Lista base (CorrecaoFinal) está vazia. Isso pode ser normal se a lista original não tinha itens elegíveis. (separacao8.js)");
    }
    ...
    mapItensBase.forEach((itemBase, codigo) => {
        ...
        if (itemSeparacao) {
            ...
            if (qtdDesejadaSeparacao <= qtdDisponivel) {
                ...
                qtdCompraAdicional = parseFloat(itemBase.quantidadeCompraAdicional || 0); // Mantém o que já faltava
                if (qtdRealASeparar >= parseFloat(itemBase.quantidade || 0) ) { // Se o que separamos já atende a quantidade total do item
                     qtdCompraAdicional = 0; // Zera a necessidade de compra que vinha do carregamento inicial.
                } else {
                    // Se o que separamos é menor que a quantidade total do item, a compra adicional é a diferença
                    // para a quantidade total, menos o que já foi separado de outras fontes (além do disponível aqui).
                    // Esta parte pode ser complexa se o itemBase.quantidadeCompraAdicional já considera outras coisas.
                    // Simplificando: se desejado > disponível, a compra é a diferença. Senão, é o que já estava.
                    // A lógica mais precisa é: Compra = QtdTotalItem - QtdRealASeparar (do estoque) - QtdJaRecebida (se aplicável e não no disponível)
                     qtdCompraAdicional = Math.max(0, parseFloat(itemBase.quantidade || 0) - qtdRealASeparar - (parseFloat(itemBase.quantidadeRecebida || 0) > necessidadeItem ? necessidadeItem : parseFloat(itemBase.quantidadeRecebida || 0)) );
                     // Esta lógica de compra precisa ser bem definida. Por ora, a qtdCompraAdicional do carregamento inicial é mantida se não houver necessidade extra pela separação.
                }

            } else { // qtdDesejadaSeparacao > qtdDisponivel
                statusComparacao = "Comprar Adicional";
                qtdRealASeparar = qtdDisponivel;
                qtdCompraAdicional = qtdDesejadaSeparacao - qtdDisponivel; // Compra o que excedeu o disponível
            ...
            qtdCompraAdicional = parseFloat(itemBase.quantidadeCompraAdicional || 0); // Mantém a compra adicional calculada na carga
        }
        ...
        itensProcessados.push({
            ...
            itemCompraTotal: itemBase.itemCompraTotal || false,
        });
    });
    ...
    if (itensProcessados.length === 0) {
        console.warn("[compararEProcessarListas] NENHUM item processado para CorrecaoFinal (separacao8.js).");
    }
...
async function gerarSeparacaoComComparacao() {
    ...
    try {
        ...
        console.error("Erro em gerarSeparacaoComComparacao (separacao8.js):", error);
...
async function processarXLSX(data) {
    if (typeof XLSX === 'undefined') {
        console.error("A biblioteca XLSX (SheetJS) não está carregada (separacao8.js).");
...
// FIM DO ARQUIVO js/separacao8.js
// ESTE É O ARQUIVO SEPARACAO8.JS
