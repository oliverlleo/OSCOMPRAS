// INÍCIO DO ARQUIVO js/separacao3.js
// ESTE É O ARQUIVO SEPARACAO3.JS (para verificação e teste)

let tabelaCorrecao = null; // Variável global para a DataTable

document.addEventListener("DOMContentLoaded", () => {
    console.log("DOM completamente carregado (separacao3.js).");
    ...
    document.getElementById("selectLista").addEventListener("change", async () => {
        ...
        if (clienteId && tipoProjeto && nomeListaOriginal) {
            await carregarItensParaSeparacaoNaTabela(clienteId, tipoProjeto, nomeListaOriginal);
        }
    });
    ...
    if (btnGerarNecessidade) {
        btnGerarNecessidade.addEventListener("click", async () => {
            ...
            try {
                ...
                console.error("Erro ao carregar itens para necessidade de compra (separacao3.js):", error);
    ...
    document.getElementById("btnConfirmarNecessidade").addEventListener("click", async () => {
        ...
        try {
            ...
            console.error("Erro ao confirmar necessidade de compra (separacao3.js):", error);
    ...
    if (typeof $ !== "undefined" && $.fn && $.fn.DataTable) {
        ...
        console.error("jQuery ou DataTables não estão carregados (separacao3.js).");
    }
});
...
function carregarClientes() {
    ...
    firebase.database().ref("clientes").once("value")
        ...
        }).catch((err) => console.error("Erro ao carregar clientes (separacao3.js):", err));
}
...
function carregarTiposProjeto() {
    ...
    firebase.database().ref(`projetos/${clienteId}`).once("value")
        .then((snap) => {
            ...
            if (typeof $ !== "undefined" && $.fn && $.fn.select2 && $(sel).data("select2")) $(sel).trigger("change");
        }).catch((err) => console.error("Erro ao carregar tipos de projeto (separacao3.js):", err));
}
...
async function carregarListas() {
    ...
    try {
        ...
        console.error("Erro ao carregar listas (separacao3.js):", err);
        if (typeof mostrarNotificacao === "function") mostrarNotificacao("Falha ao carregar listas.", "danger");
    }
}

async function carregarItensParaSeparacaoNaTabela(clienteId, tipoProjeto, nomeListaOriginal) {
    if (!clienteId || !tipoProjeto || !nomeListaOriginal) {
        if (tabelaCorrecao) tabelaCorrecao.clear().draw();
        return;
    }
    console.log(`[carregarItensParaSeparacaoNaTabela] Para ${clienteId}/${tipoProjeto}/${nomeListaOriginal} (separacao3.js)`);
    try {
        ...
        arrayDeItensOriginais.forEach((it, index) => {
            ...
            if (condicaoEmpenho || condicaoRecebido) {
                ...
                if (quantidadeDisponivel >= quantidadeItem) {
                    status = "Pronto para Separar (Total)";
                } else if (quantidadeDisponivel > 0) {
                    status = "Pronto para Separar (Parcial)";
                } else {
        ...
        if (tabelaCorrecao) {
            tabelaCorrecao.clear().rows.add(itensParaExibir).draw();
        }
        if (itensParaExibir.length === 0 && typeof mostrarNotificacao === "function") {
            mostrarNotificacao("Nenhum item elegível para separação nesta lista.", "info");
        }
    } catch (error) {
        console.error("Erro em carregarItensParaSeparacaoNaTabela (separacao3.js):", error);
        if (typeof mostrarNotificacao === "function") mostrarNotificacao("Erro ao carregar itens para separação.", "danger");
    }
}
...
async function compararEProcessarListas(clienteId, tipoProjeto, nomeListaOriginal) {
    console.log(`[compararEProcessarListas] Iniciando para ${clienteId}/${tipoProjeto}/${nomeListaOriginal} (separacao3.js)`);
    ...
    if (Array.isArray(dadosListaOriginal.itens)) listaOriginalItensRaw = dadosListaOriginal.itens;
    else if (Array.isArray(dadosListaOriginal)) listaOriginalItensRaw = dadosListaOriginal;
    else if (typeof dadosListaOriginal === 'object' && dadosListaOriginal !== null) {
    ...
    if (listaOriginalItens.length === 0) console.warn("[compararEProcessarListas] Lista original de PROJETOS vazia ou inválida (separacao3.js).");
    ...
    listaOriginalItens.forEach(item => {
        ...
        mapListaOriginal.set(codigo, { ...item, empenho, quantidadeRecebida, quantidadeDisponivelOriginal: empenho + quantidadeRecebida });
    });
    ...
    mapListaOriginal.forEach((itemOriginal, codigo) => {
        ...
        const qtdDisponivel = itemOriginal.quantidadeDisponivelOriginal || 0;
        ...
        if (itemSeparacao) {
            ...
            const quantidadeOriginalItem = parseFloat(itemOriginal.quantidade || 0);
            qtdCompraAdicional = Math.max(0, quantidadeOriginalItem - qtdDisponivel);
        }
    ...
    mapListaSeparacao.forEach((itemSeparacao, codigo) => {
        if (!mapListaOriginal.has(codigo)) {
            let qtdDesejadaSeparacaoInterna = parseFloat(itemSeparacao.quantidade) || 0; // Renomeada para evitar conflito de escopo
            itensProcessados.push({
                ...
                quantidadeDesejadaSeparacao: qtdDesejadaSeparacaoInterna, // Usando a variável renomeada
                quantidadeDisponivelOriginal: 0,
                quantidadeParaSepararReal: 0,
                quantidadeCompraAdicional: qtdDesejadaSeparacaoInterna, // Usando a variável renomeada
                quantidadeDevolucaoEstoque: 0,
    ...
    if (itensProcessados.length === 0) {
        console.warn("[compararEProcessarListas] NENHUM item processado para CorrecaoFinal (separacao3.js).");
    }
...
async function gerarSeparacaoComComparacao() {
    ...
    try {
        ...
        console.error("Erro em gerarSeparacaoComComparacao (separacao3.js):", error);
...
async function processarXLSX(data) {
    if (typeof XLSX === 'undefined') {
        console.error("A biblioteca XLSX (SheetJS) não está carregada (separacao3.js).");
...
// FIM DO ARQUIVO js/separacao3.js
// ATUALIZADO (este é o separacao3.js)
