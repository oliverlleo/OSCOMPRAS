...
function atualizarDashboardAvancado() {
    try {
        // Atualiza o gráfico de entregas por status
        if (chartEntregas) {
            const entregasPendentes = parseInt(document.getElementById('entregasPendentes')?.textContent || '0');
            const entregasRecebidas = parseInt(document.getElementById('entregasRecebidas')?.textContent || '0');
            const entregasIncorretas = parseInt(document.getElementById('entregasIncorretas')?.textContent || '0');
            
            chartEntregas.data.datasets[0].data = [entregasPendentes, entregasRecebidas, entregasIncorretas];
            chartEntregas.update();
        }
        
        // Verifica se todosItens existe e tem elementos
