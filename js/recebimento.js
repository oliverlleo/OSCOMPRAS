...
function configurarEventListeners() {
    ...
    // Event listeners para os filtros (usando jQuery para compatibilidade com Select2)
    const $filtroFornecedor = $("#filtroFornecedor");
    if ($filtroFornecedor.length) {
        $filtroFornecedor.on("change", function() {
            console.log("Filtro Fornecedor alterado para:", this.value);
    ...
    if ($filtroCliente.length) {
        $filtroCliente.on("change", function() {
            console.log("Filtro Cliente alterado para:", this.value);
            filtroAtual.cliente = this.value;
            aplicarFiltros();
        });
    }
    
    const $filtroCodigo = $("#filtroCodigo"); // Mantém addEventListener pois é input normal
    if ($filtroCodigo.length) {
        $filtroCodigo[0].addEventListener("input", function() {
            // console.log("Filtro Código alterado para:", this.value); // Log opcional
                                            ...
                                            lista.forEach((item, index) => {
                                                ...
                                                if (
                                                    ...
                                                    const itemComReferencias = {
                                                        ...
                                                        clienteId: clienteId,
                                                        clienteNome: cliente.nome,
                                                        prazoEntregaCliente: cliente.prazoEntrega, // Adicionado conforme instrução
                                                        projetoId: projetoNome,
                                                        projetoNome: projetoNome,
                                                        listaId: listaId
                                                    };
