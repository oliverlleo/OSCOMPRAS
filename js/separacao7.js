// INÍCIO DO ARQUIVO js/separacao7.js
// ESTE É O ARQUIVO SEPARACAO7.JS (para verificação e teste final)

let tabelaCorrecao = null; // Variável global para a DataTable

document.addEventListener("DOMContentLoaded", () => {
    console.log("DOM completamente carregado (separacao7.js).");
    ...
    if (btnGerarNecessidade) {
        btnGerarNecessidade.addEventListener("click", async () => {
            ...
            try {
                ...
                console.error("Erro ao carregar itens para necessidade de compra (separacao7.js):", error);
    ...
    document.getElementById("btnConfirmarNecessidade").addEventListener("click", async () => {
        ...
        try {
            ...
            console.error("Erro ao confirmar necessidade de compra (separacao7.js):", error);
    ...
    if (typeof $ !== "undefined" && $.fn && $.fn.DataTable) {
        ...
        console.error("jQuery ou DataTables não estão carregados (separacao7.js).");
    }
});
...
function carregarClientes() {
    ...
    firebase.database().ref("clientes").once("value")
        ...
        }).catch((err) => console.error("Erro ao carregar clientes (separacao7.js):", err));
}
...
function carregarTiposProjeto() {
    ...
    firebase.database().ref(`projetos/${clienteId}`).once("value")
        .then((snap) => {
            ...
            if (typeof $ !== "undefined" && $.fn && $.fn.select2 && $(sel).data("select2")) $(sel).trigger("change");
        }).catch((err) => console.error("Erro ao carregar tipos de projeto (separacao7.js):", err));
}
...
async function carregarListas() {
    ...
    try {
        ...
        console.error("Erro ao carregar listas (separacao7.js):", err);
...
async function processarListaSelecionadaParaTabela(clienteId, tipoProjeto, nomeListaOriginal) {
    ...
    console.log(`[processarListaSelecionadaParaTabela] Para ${clienteId}/${tipoProjeto}/${nomeListaOriginal} (separacao7.js)`);
    try {
        ...
        if (Array.isArray(dadosListaOriginal.itens)) {
            arrayDeItensOriginais = dadosListaOriginal.itens;
        } else if (Array.isArray(dadosListaOriginal)) { // Caso a lista seja diretamente um array de itens
            arrayDeItensOriginais = dadosListaOriginal;
        } else if (typeof dadosListaOriginal === 'object' && dadosListaOriginal !== null) {
            // Caso seja um objeto onde cada chave é um item (padrão Firebase para listas com push)
        ...
        arrayDeItensOriginais.forEach((it, index) => {
            ...
            console.log(`[processarListaSelecionadaParaTabela] Avaliando item ${it.codigo}: Qtd=${quantidadeItem}, Emp=${empenhoItem}, Nec=${necessidadeItem}, QtdRec=${quantidadeRecebidaItem}`);
            ...
            console.log(`  CondEmp: ${condicaoEmpenho}, CondRec: ${condicaoRecebido}`);
            ...
            if (condicaoEmpenho || condicaoRecebido) {
                ...
                console.log(`  > Item ${it.codigo} ELEGÍVEL. Adicionado para CorrecaoFinal. Formatado:`, JSON.parse(JSON.stringify(itemFormatado)));
            } else {
                console.log(`  > Item ${it.codigo} NÃO ELEGÍVEL.`);
            }
        });

        const refCorrecaoFinal = firebase.database().ref(`CorrecaoFinal/${clienteId}/${tipoProjeto}/${nomeListaOriginal}/itensProcessados`);
        await refCorrecaoFinal.set(itensParaCorrecaoFinal);
        console.log(`[processarListaSelecionadaParaTabela] ${itensParaCorrecaoFinal.length} itens elegíveis salvos em CorrecaoFinal. Conteúdo:`, JSON.parse(JSON.stringify(itensParaCorrecaoFinal)));
        ...
        console.error("Erro em processarListaSelecionadaParaTabela (separacao7.js):", error);
...
async function compararEProcessarListas(clienteId, tipoProjeto, nomeListaOriginal) {
    console.log(`[compararEProcessarListas] Iniciando para ${clienteId}/${tipoProjeto}/${nomeListaOriginal} (separacao7.js)`);
    ...
    if (itensBase.length === 0) {
        console.warn("[compararEProcessarListas] Lista base (CorrecaoFinal) está vazia. Nada para comparar. (separacao7.js)");
    }
    ...
    if (itensProcessados.length === 0) {
        console.warn("[compararEProcessarListas] NENHUM item processado para CorrecaoFinal (separacao7.js).");
    }
...
async function gerarSeparacaoComComparacao() {
    ...
    try {
        ...
        console.error("Erro em gerarSeparacaoComComparacao (separacao7.js):", error);
...
async function processarXLSX(data) {
    if (typeof XLSX === 'undefined') {
        console.error("A biblioteca XLSX (SheetJS) não está carregada (separacao7.js).");
...
// FIM DO ARQUIVO js/separacao7.js
// ESTE É O ARQUIVO SEPARACAO7.JS
