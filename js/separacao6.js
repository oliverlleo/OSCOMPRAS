// INÍCIO DO ARQUIVO js/separacao6.js
// ESTE É O ARQUIVO SEPARACAO6.JS (para verificação e teste final)

let tabelaCorrecao = null; // Variável global para a DataTable

document.addEventListener("DOMContentLoaded", () => {
    console.log("DOM completamente carregado (separacao6.js).");
    ...
    if (btnGerarNecessidade) {
        btnGerarNecessidade.addEventListener("click", async () => {
            ...
            try {
                ...
                console.error("Erro ao carregar itens para necessidade de compra (separacao6.js):", error);
    ...
    document.getElementById("btnConfirmarNecessidade").addEventListener("click", async () => {
        ...
        try {
            ...
            console.error("Erro ao confirmar necessidade de compra (separacao6.js):", error);
    ...
    if (typeof $ !== "undefined" && $.fn && $.fn.DataTable) {
        ...
        console.error("jQuery ou DataTables não estão carregados (separacao6.js).");
    }
});
...
function carregarClientes() {
    ...
    firebase.database().ref("clientes").once("value")
        ...
        }).catch((err) => console.error("Erro ao carregar clientes (separacao6.js):", err));
}
...
function carregarTiposProjeto() {
    ...
    firebase.database().ref(`projetos/${clienteId}`).once("value")
        .then((snap) => {
            ...
            if (typeof $ !== "undefined" && $.fn && $.fn.select2 && $(sel).data("select2")) $(sel).trigger("change");
        }).catch((err) => console.error("Erro ao carregar tipos de projeto (separacao6.js):", err));
}
...
async function carregarListas() {
    ...
    try {
        ...
        console.error("Erro ao carregar listas (separacao6.js):", err);
...
async function processarListaSelecionadaParaTabela(clienteId, tipoProjeto, nomeListaOriginal) {
    ...
    console.log(`[processarListaSelecionadaParaTabela] Para ${clienteId}/${tipoProjeto}/${nomeListaOriginal} (separacao6.js)`);
    try {
        ...
        console.error("Erro em processarListaSelecionadaParaTabela (separacao6.js):", error);
...
async function compararEProcessarListas(clienteId, tipoProjeto, nomeListaOriginal) {
    console.log(`[compararEProcessarListas] Iniciando para ${clienteId}/${tipoProjeto}/${nomeListaOriginal} (separacao6.js)`);
    ...
    if (itensBase.length === 0) {
        console.warn("[compararEProcessarListas] Lista base (CorrecaoFinal) está vazia. Nada para comparar. (separacao6.js)");
    }

    const mapItensBase = new Map();
    itensBase.forEach(item => {
        if (item && typeof item.codigo !== 'undefined') {
            // Garante que quantidadeDisponivelOriginal esteja presente e seja numérico
    ...
    mapItensBase.forEach((itemBase, codigo) => {
        ...
        let statusComparacao = itemBase.statusComparacao; // Usa o status inicial como base

        if (itemSeparacao) {
            statusComparacao = ""; // Reseta para recalcular
            ...
            // quantidadeCompraAdicional já foi calculada em processarListaSelecionadaParaTabela
            // e está em itemBase.quantidadeCompraAdicional. Mantém esse valor.
        ...
        itensProcessados.push({
            ...
            quantidadeDisponivelOriginal: qtdDisponivel, // Mantém o que foi carregado inicialmente
            ...
            // itemCompraTotal não deve ser alterado aqui, só para itens novos
    ...
    if (itensProcessados.length === 0) {
        console.warn("[compararEProcessarListas] NENHUM item processado para CorrecaoFinal (separacao6.js).");
    }
...
async function gerarSeparacaoComComparacao() {
    ...
    try {
        ...
        console.error("Erro em gerarSeparacaoComComparacao (separacao6.js):", error);
...
async function processarXLSX(data) {
    if (typeof XLSX === 'undefined') {
        console.error("A biblioteca XLSX (SheetJS) não está carregada (separacao6.js).");
...
// FIM DO ARQUIVO js/separacao6.js
// ESTE É O ARQUIVO SEPARACAO6.JS
